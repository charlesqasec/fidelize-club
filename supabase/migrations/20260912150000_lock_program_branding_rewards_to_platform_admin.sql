...
-- estabelecimento (2026-09-12) — regra oficial do MVP: "Programa, regras,
-- identidade visual e recompensas: estabelecimento consulta; Fidelize Admin
-- configura." O painel do estabelecimento não deve ser um mini Fidelize
-- Admin.
--
-- Correção: as 5 RPCs de Programa/Branding/Recompensas de 4.6C
-- (`20260911120000_program_branding_rewards_admin_rpcs.sql`) aceitavam
-- OWNER além de PLATFORM_ADMIN. `CREATE OR REPLACE` mantém assinatura,
-- comentário, grants (`authenticated`, sem `anon`) e todo o resto do corpo
-- idênticos — muda só o portão de autorização (`v_actor <> 'PLATFORM_ADMIN'`
-- em vez de `not in ('PLATFORM_ADMIN', 'OWNER')`). Nenhuma tabela, policy ou
-- trigger é tocada.
--
-- RBAC infra preservada de propósito: `admin_org_actor_role` continua
-- resolvendo OWNER/MANAGER/STAFF normalmente — só o portão de cada RPC
-- muda. Reabrir para OWNER no futuro é reverter só esta migration.
...
comment on function public.admin_update_program(uuid, text, text, jsonb) is
  'ETAPA 4.6F (era 4.6C). Atualiza nome/status/rules (whitelist por type) de loyalty_programs. Somente PLATFORM_ADMIN — regra do MVP pos-homologacao: estabelecimento so consulta programa/branding/recompensas. type nunca editavel. Registra audit_logs (program.updated).';

create or replace function public.admin_upsert_program_branding(
  p_program_id      uuid,
...
comment on function public.admin_upsert_program_branding(uuid, text, text, text, text, text, text, text, text) is
  'ETAPA 4.6F (era 4.6C). Cria/atualiza program_branding (1:1). Somente PLATFORM_ADMIN — regra do MVP pos-homologacao: estabelecimento so consulta identidade visual. Registra audit_logs (program.branding_updated).';
...
comment on function public.admin_create_reward(uuid, text, text, text, integer) is
  'ETAPA 4.6F (era 4.6C). Cria uma recompensa (rewards). Somente PLATFORM_ADMIN — regra do MVP pos-homologacao. Registra audit_logs (reward.created).';
...
comment on function public.admin_update_reward(uuid, text, text, text, integer) is
  'ETAPA 4.6F (era 4.6C). Atualiza uma recompensa. Somente PLATFORM_ADMIN — regra do MVP pos-homologacao. Registra audit_logs (reward.updated).';

create or replace function public.admin_set_reward_status(
  p_reward_id uuid,
  p_status    text
)
returns jsonb
...
comment on function public.admin_set_reward_status(uuid, text) is
  'ETAPA 4.6F (era 4.6C). Ativa/pausa/arquiva uma recompensa. Somente PLATFORM_ADMIN — regra do MVP pos-homologacao. Registra audit_logs (reward.status_changed).';
